...
sub test {
  ...
  for (1..$iter) {
    ...
    @rpp = Math::Prime::Util::GMP::allrootmod($a,$k,$n);
    die "allrootmod fail for $a,$k,$n\n" unless vecequal(\@rxs,\@rpp);
    if ($k == 2) {
      @sxs = allsqrtmod($a,$n);
      @spp = Math::Prime::Util::GMP::allsqrtmod($a,$n);
