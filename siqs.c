...
typedef struct {
  ...
  uint32_t multiplier_refine_divisor;
...
static void siqs_resolve_policy(siqs_policy_t *policy, uint32_t bits,
  ...
  policy->multiplier_refine_divisor = band->multiplier_refine_divisor;
...
static void siqs_select_parameters(siqs_parameters_t *p, const mpz_t n,
  ...
  p->multiplier_refine_divisor = policy.multiplier_refine_divisor;
...
static void siqs_resieve_candidates(siqs_ctx_t *ctx,
  ...
  for (c = 0; c < ctx->candidate_count; c++) {
    uint32_t pos = (uint32_t)(candidates[c].x + half_interval);
    for (i = factor_begin; i < barrett_end; i++) {
      uint32_t rem = siqs_reduce_u32(pos, prime[i], reciprocal[i]);
...
static void siqs_ctx_init(siqs_ctx_t *ctx, const mpz_t original,
  ...
  ctx->multiplier = siqs_choose_multiplier(
      n, ctx->params.fb_size, ctx->params.multiplier_refine_divisor);
