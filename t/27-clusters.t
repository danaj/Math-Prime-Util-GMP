...
use Test::More;
use Math::Prime::Util::GMP qw/sieve_prime_cluster is_prime sieve_primes sieve_twin_primes/;
...
my $small = [ [sieve_primes($sbeg,$send)], [sieve_twin_primes($sbeg,$send)] ];
my $large = [
              [map { Math::BigInt->new(''.$_) } sieve_primes($mbeg,$mend)],
              [map { Math::BigInt->new(''.$_) } sieve_twin_primes($mbeg,$mend)]
            ];
...
for my $test (@high_check) {
  my($n,$name,$cl) = @$test;
  my $delta = Math::BigInt->new(1000000);
  my @res = sieve_prime_cluster($n-$delta, $n+$delta, @$cl);
